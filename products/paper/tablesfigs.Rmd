...
```{r hdi-effect, fig.cap="Scatterplot showing the relationship between the Human Development Index in a country and the association between education and support for violence practices in a country. The association is estimated from Model 2 of Table 1 and controls for differences in individual-level characteristics across countries. Lines show the best OLS fit to the scatterplot." }
country_effects$ss_africa <- factor(country_effects$region=="Africa",
                                    levels=c(TRUE,FALSE),
                                    labels=c("Sub-Saharan Africa",
                                             "All Other Countries"))
ggplot(country_effects, aes(x=hdi, y=educqq_model2))+
  geom_point(aes(color=ss_africa))+
  geom_hline(yintercept = 0, linetype=2)+
  #geom_smooth(method="lm", se=FALSE, color="grey20")+
  geom_smooth(method="lm", se=FALSE, aes(group=ss_africa, color=ss_africa),
              linetype=2)+
  geom_text_repel(aes(label=country), size=3)+
  scale_color_viridis_d(end=0.75)+
  labs(x="human development index",
       y="education slope in country",
       color="Region")+
  theme_mine()
```
