...
cor_violent <- polychoric(violent, global=FALSE)$rho

psych::alpha(cor_violent)

loadings(fa(cor_violent,1,rotate=rotation_choice))
fa.diagram(fa(cor_violent,1,rotate=rotation_choice))
loadings(fa(cor_violent,2,rotate=rotation_choice))
fa.diagram(fa(cor_violent,2,rotate=rotation_choice))
loadings(fa(cor_violent,3,rotate=rotation_choice))
fa.diagram(fa(cor_violent,3,rotate=rotation_choice))
```
...
Lets go back and calculate $\alpha$ for the two separate indices.

```{r}
cor_moral_death <- polychoric(morality[,c("moral_euthansia","moral_suicide",
                                          "moral_abortion")])$rho

psych::alpha(cor_moral_death)
```

```{r}
cor_moral_sex <- polychoric(morality[,c("moral_prostitution","moral_premar_sex",
                                          "moral_gay")])$rho

psych::alpha(cor_moral_sex)
