...
load("output/pewdata_combined.RData")
library(psych)
library(corrgram)
...
I am measuring ant-secularism here as a preference for Islam and Islamic doctrine to occupy a privileged position within government, law, and civil society. We have several candidate variables, but also some significant missing values by country.

+------------------------+---------------------------------------+---------------------------+
| Name                   | Question                              | Missing Countries         |
+========================+=======================================+===========================+
| islamic_parties (Q13)  |Islamic parties better/worse than other| - Albania                 |
|                        |political parties?                     | - Iran                    |
|                        |                                       | - Russia                  |
|                        |                                       | - Thailand                |
|                        |                                       | - Uzbekistan              |
|                        |                                       | - Sub-Saharan Africa      |
+------------------------+---------------------------------------+---------------------------+
| leaders_influence (Q15)|How much influence should religious    | - Morocco                 |
|leaders_influence_binary|leaders have in political matters?     | - Thailand                |
|                        |In Sub-saharan African sample, this was| - Uzbekistan              |
|                        |asked slightly differently and R only  |                           |
|                        |given binary prefrence, so we also     |                           |
|                        |create binary version of variable      |                           |
+------------------------+---------------------------------------+---------------------------+
| sharia_law2(Q79a,80,81)|Do you favor making Sharia the official| - Uzbekistan              |
| sharia_law3            |law of the land? Q81 further determines| - Wording slightly        |
|                        |whether R wants universal Sharia or    |  different in Russia,     |
|                        |only for Muslims. This question is not |  Thailand, and Iran       |
|                        |asked for SS African sample, so we     |                           |
|                        |use binary and trinary variables       |                           |
+------------------------+---------------------------------------+---------------------------+
| relig_judge (Q92a)     |Give muslim leaders and religious      | - Iran                    |
|                        |judges the power to decide family and  | - Morocco                 |
|                        |property disputes.                     | - Uzbekistan              |
|                        |                                       | - Wording slightly        |
|                        |                                       |different in Russia,       |
|                        |                                       |Thailand, and Turkey       |
+------------------------+---------------------------------------+---------------------------+
...
```{r anti_secularism}
anti_secular <- cbind(islamic_parties=as.numeric(pew$islamic_parties),
                  leaders_influence_binary=as.numeric(pew$leaders_influence_binary),
                  sharia_official=as.numeric(pew$sharia_law2),
                  relig_judge=as.numeric(pew$relig_judge))
corrgram(anti_secular, upper.panel="panel.cor", order="PCA")

alpha(cor(anti_secular, use="pairwise.complete.obs"))
nfactors(cor(anti_secular, use="pairwise.complete.obs"))

loadings(fa(cor(anti_secular, use="pairwise.complete.obs"),1,rotate=rotation_choice))
fa.diagram(fa(cor(anti_secular, use="pairwise.complete.obs"),1,rotate=rotation_choice))
loadings(fa(cor(anti_secular, use="pairwise.complete.obs"),2,rotate=rotation_choice))
fa.diagram(fa(cor(anti_secular, use="pairwise.complete.obs"),2,rotate=rotation_choice))

#without islamic parties, since that is the weakest and missing for SS Africa
vss(cor(anti_secular[,-1], use="pairwise.complete.obs"))
loadings(fa(cor(anti_secular[,-1], use="pairwise.complete.obs"),1,rotate=rotation_choice))
fa.diagram(fa(cor(anti_secular[,-1], use="pairwise.complete.obs"),1,rotate=rotation_choice))
fa.diagram(fa(cor(anti_secular[,-1], use="pairwise.complete.obs"),2,rotate=rotation_choice))
```

#### Summary

The three variables of leaders_influence_binary, sharia_law2, and relig_judge hang together pretty  well, with a decent Cronbach's $\alpha$ and good factor loadings. There might be some argument that leaders_influence_binary is not as correlated as highly as the others and that it fits better in a two-factor model with islamic_parties, but it works pretty well and islamic_parties is not available for the SS African sample. I also worry that islamic_parties might may be very country specific depending on the actual Islamist parties and non-Islamist parties in a country. 

Based on this I would recommend we use the three variables of leaders_influence_binary, sharia_law2, and relig_judge to construct a scale. 


### Women's Rights
<a href="#top">Back to top</a>

Attitudes towards women's rights and women's equality. I think we are going to have to drop this one anyway because we are missing three of four variables in SS Africa, but I will leave it here. 

+------------------------+---------------------------------------+---------------------------+
| Name                   | Question                              | Missing Countries         |
+========================+=======================================+===========================+
| right_veil (Q58)       |Women should have the right to decide  | - Iran                    |
|                        |to veil (reverse-coded)                |                           |
+------------------------+---------------------------------------+---------------------------+
| right_divorce (Q77)    |Wife should have right to divorce      | - Afghanistan             |
|                        |husband (reverse-coded)                | - Iran                    |
|                        |                                       | - Sub-Saharan Africa      |
+------------------------+---------------------------------------+---------------------------+
| obey_husband (Q78)     |A wife must always ovey her husband    | - Iran                    |
|                        |                                       | - Sub-Saharan Africa      |
+------------------------+---------------------------------------+---------------------------+
| sons_inherit (Q83 )    |Sons should have greater right than    | - Iran                    |
|                        |daughters to inheritance.              | - Sub-Saharan Africa      |
+------------------------+---------------------------------------+---------------------------+

```{r womens_rights}
womensrights <- cbind(right_veil=as.numeric(pew$right_veil),
                      right_divorce=as.numeric(pew$right_divorce),
                      obey_husband=as.numeric(pew$obey_husband),
                      sons_inherit=as.numeric(pew$sons_inherit))
corrgram(womensrights, upper.panel="panel.cor", order="PCA")

alpha(cor(womensrights, use="pairwise.complete.obs"))

nfactors(cor(womensrights, use="pairwise.complete.obs"))

loadings(fa(cor(womensrights, use="pairwise.complete.obs"),1,rotate=rotation_choice))
fa.diagram(fa(cor(womensrights, use="pairwise.complete.obs"),1,rotate=rotation_choice))
loadings(fa(cor(womensrights, use="pairwise.complete.obs"),2,rotate=rotation_choice))
fa.diagram(fa(cor(womensrights, use="pairwise.complete.obs"),2,rotate=rotation_choice))

#hmm suggests two separate factors, one around issues of "right to" and the other around conservative values
```

#### Summary

We have a couple of difficulties with this construct. First the data are missing on three of the four variables for SS Africa, meaning our analysis would probably be limited to the non-African sample. Second, the alpha and factor loadings for one factor are somewhat weak.

Based on these results, I would recommend we drop this as a dependent variable for this paper.

### Separation of Dependent Variables
<a href="#top">Back to top</a>

Now lets combine the dependent variables for violent practices and anti-secularism, in cases where we are not missing SS Africa and see how come out in a combined EFA.

```{r separate_dependent_corrgram, echo=FALSE}

dv <- cbind(violent[,3:6], anti_secular[,-1])

corrgram(dv, upper.panel="panel.cor", order="PCA")
```

Map out the factor loadings when I use three factors.

```{r separate_dependent_loadings, echo=FALSE}
nfactors(cor(dv, use="pairwise.complete.obs"))

fa_dv <- fa(cor(dv, use="pairwise.complete.obs"),3,rotate=rotation_choice)
loadings(fa_dv)

plot(fa_dv, labels=colnames(dv), show.points=FALSE,
     cluster=c(1,1,1,2,3,3,3))
fa.diagram(fa_dv)

#what happens with two and four
fa_dv <- fa(cor(dv, use="pairwise.complete.obs"),2,rotate=rotation_choice)
loadings(fa_dv)
fa.diagram(fa_dv)

fa_dv <- fa(cor(dv, use="pairwise.complete.obs"),4,rotate=rotation_choice)
loadings(fa_dv)
fa.diagram(fa_dv)
```
...
There is a very clear and obvious separation into two multi-item factors measuring support for violent practices and anti-secularism, with the civilian_target question as distinct. The only slightly ambigious question here is the leaders_influence_binary item which had a bit weaker factor loading on anti-secularism than the other two items. Based on this, I recommend that we use three dependent variables for the analysis:

- **support for violent prcactices** scale composed of death_apostasy, severe_corporal, and stone_adultery.
- **anti-secular views** scale composed of leaders_influence_binary, sharia_official, and relig_judge.
- **support for terrorism** scale from civilian_target variable. 
